...
interface UseApprovalsWithSubscriptionOptions {
  sessionId?: string;
  pollInterval?: number; // milliseconds, default 5000
  enabled?: boolean; // default true
}

export function useApprovalsWithSubscription(options: UseApprovalsWithSubscriptionOptions = {}) {
  const { sessionId, pollInterval = 5000, enabled = true } = options;
  ...
  }, [fetchApprovals, loading, error, enabled, pollInterval]);
